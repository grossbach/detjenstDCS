...
Die unterschiedlichen Anzahlen ausgewerteter Tonleitern in der Spalte *N* ist erklärbar durch den Datenverlust bei PatientIn P05, lediglich am ersten Tag vor der Stimulation sind ca. 130 Tonleitern mehr gespielt worden als in den folgenden Bedingungen ohne Datenverluste.
...
`r kfigr::figr("OverviewTabIOIsP04", type = TABREF)`. *Lage- und Streuparameter der Inter-onset Intervalle von P04 nach Tag und Zeitpunkt (in ms).*
...
Der Daten der Follow-up-Messung müssen aus der Analyse ausgeschlossen werden, da es sonst zur Konfundierung von Tag und Zeitpunkt kommt. Die Ergebnisse der modellierten Ergebnisse können hinterher aber mit den Rohwerten der mittleren SDs im Follow-up vergleichen werden.
...
Das hier zur Modellierung verwendete $\text{R}$-Paket brms [@Buerkner2017;@Buerkner2018] verwendet in der aktuellen Version (2.15.0) für Populationseffekte (außer dem Interzept) keine Priors (diese können aber händisch gesetzt werden). Variierende Interzepte erhalten automatisch eine $\cal{student\text{_}t}(3, 0, s)$-verteilte Prior, wobei $s$ für die Standardabweichung der Normalverteilung steht. Diese Prior muss bei kleinen Stichproben und/oder sehr großer Variation strenger sein--$s$ muss also kleiner gewählt werden--, damit das Modell konvergieren kann, weshalb hier eine um Null zentrierte Normalverteilung mit einer Standardabweichung von 1 zum Einsatz kommt (`r kfigr::figr("priors_null", type = PRIORREF)`). 
...
```{r}
post_null <- brms::posterior_samples(m_null)
...
Es gibt anscheinend einen schwachen Tageseffekt (in der Vorher-Messung bei der ersten Abwärtstonleiter).
...
Interessanterweise gibt es in diesem Modell (also am ersten Tag!) keinen Vorher-nacher-Effekt (in der ersten Abwärtstonleiter).
...
Am ersten Tag gab es in der Vorher-Messung einen "Effekt" der Spielrichtung in der ersten Tonleiter. Die PatientInnen waren also im Mittel in der Abwärtsrichtung stäker betroffen (die Standardabweichung war größer) als beim Aufwärtsspiel.
...
Im Mittel nahm die Standardabweichung im Verlauf einer Messung (hier: erster Tag, prae interventionem, erste Abwärtstonleiter) zu.

#### Tag x Zeitpunkt + Aufwärts + Skala + (1 | ID)

In diesem Abschnitt wird die erste Interaktion modelliert, sodass die Steigungen von *Zeitpunkt* für jeden Tag erittelt werden.
...
(form_Day_x_Time_Ascend_Scale__1_ID <- brms::bf(SD ~ 1 + Day * Time + Ascend + Scale +
                                                (1 | ID), 
                                              family = brms::skew_normal()))
...
(form_Day_x_Time_x_Ascend_Scale__1_ID <- brms::bf(SD ~ 1 + Day * Time * Ascend + Scale +
                                                (1 | ID), 
                                              family = brms::skew_normal()))
...
(form_Day_x_Time_x_Ascend_Scale__1_ID__1_Day <- brms::bf(SD ~ 1 + Day * Time * Ascend + Scale +
                                                    (1 | ID) +
                                                    (1 | Day), 
                                                  family = brms::skew_normal()))
...
(form_Day_x_Time_x_Ascend_Scale__1_ID__1_Day__1_Time <- brms::bf(SD ~ 1 + Day * Time * Ascend + Scale +
                                                                   (1 | ID) +
                                                                   (1 | Day) +
                                                                   (1 | Time), 
                                                                 family = brms::skew_normal()))
...
(form_Day_x_Time_x_Ascend_Scale__1_ID__1_Day__1_Time__1_Ascend <- brms::bf(SD ~ 1 + Day * Time * Ascend + Scale +
                                                                   (1 | ID) +
                                                                   (1 | Day) +
                                                                   (1 | Time) +
                                                                   (1 | Ascend), 
...
`r kfigr::figr("ModelCompareTab")` zeigt, dass das Model *Tag + Zeit + Aufwärts + Skala + (1|ID)* am besten abschnitt. Die Differenz zu den nachfolgenden Modellen ist aber angesichts des Standardfehlers der Differenzen vernachlässigbar, sodass sich die Interpretation auf das fast komplette Modell, *Tag x Zeit x Aufwärts + Skala + (1|ID) + (1|Tag) + (1|Zeit) + (1|Aufwärts)*, beschränken sollte.

### Modellierungsergebnisse

Die Modellierungsergebnisse iḿ Abschnitt "[Tag x Zeitpunkt x Aufwärts + Skala + (1 | ID) + (1 | Tag) + (1 | Zeitpunkt) + (1 | Aufwärts)]" zeigen, dass es in den vorliegenden Daten keinen Hinweis auf eine unmittelbare Wirkung der tDC-Stimulation gibt, da es an keinem der drei Stimulationstage Vorher-Nachher-Effekte zu geben scheint, aber es gibt eine leichte Verbesserung in der Spielunregelmäßigkeit zwischen dem ersten und dem zweiten Tag (`r kfigr::figr("Day_x_Time_x_Ascend_Scale__1_ID__1_Day__1_Time__1_Ascend_CondPlot")`E). 

Diese leichten Verbesserungen scheint es über die drei Wochen nur bei den Aufwärtstonleitern zu geben; bei den Abwärtstonleitern gibt es lediglich eine leichte Verbesserung zwischen erstem und zweitem Tag (`r kfigr::figr("Day_x_Time_x_Ascend_Scale__1_ID__1_Day__1_Time__1_Ascend_CondPlot")`F).

Die Aufschlüsselung nach *Zeitpunkt* und *Aufwärts* zeigt schließlich (`r kfigr::figr("Day_x_Time_x_Ascend_Scale__1_ID__1_Day__1_Time__1_Ascend_CondPlot")`G), dass es eine divergente Entwicklung bei der Spielverbesserung gab.
