...
Der Daten der Follow-up-Messung müssen aus der Analyse ausgeschlossen werden, da es sonst zur Konfundierung von Tag und Zeitpunkt kommt. Die Ergebnisse der modellierten Ergebnisse kann hinterher aber mit den Rohwerten der mittleren SDs im Follow-up vergleichen werden.
...
brms::conditional_effects(m_Day_x_Time_Ascend_Scale__1_ID)
```
...
brms::conditional_effects(m_Day_x_Time_x_Ascend_Scale__1_ID)
```
...
brms::conditional_effects(m_Day_x_Time_x_Ascend_Scale__1_ID__1_Day)
```
...
brms::conditional_effects(m_Day_x_Time_x_Ascend_Scale__1_ID__1_Day__1_Time)
```
...
brms::conditional_effects(m_Day_x_Time_x_Ascend_Scale__1_ID__1_Day__1_Time__1_Ascend)
```
...
`r kfigr::figr("ModelCompareTab")` zeigt, dass Model *Tag + Zeit + Aufwärts + Skala + (1_ID)* am besten abschneidet. Die Differenz zu den nachfolgenden Modellen ist aber angesichts des Standardfehlers der Differenzen vernachlässigbar, sodass sich die Interpretation auf das fast komplette Modell, *Tag x Zeit x Aufwärts + Skala + (1|ID) + (1|Tag) + (1|Zeit) + (1|Aufwärts)*, beschränkt.
