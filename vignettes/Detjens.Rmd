...
ggplot2::ggplot(mdata[mdata$ID != "P04", ], ggplot2::aes(IOI, color = Scale)) +
  ...
  ggplot2::facet_grid(rows = dplyr::vars(Day.0), 
                      cols = dplyr::vars(Time)) +
  ggplot2::xlab("IOI (ms)")

```

`r kfigr::figr("OverviewPlotIOIs", type = FIGREF)`. *Dichtefunktionsschätzung der Inter-onset Intervalle aller von P01, P02, P03 und P05 gespielten Tonleitern, getrennt nach* Zeit *(pre, post, follow-up) und* Tag *(0, 7, 14 und 42).*

***

Die Verteilungen der IOIs in den sieben Bedingungen (`r kfigr::figr("OverviewPlotIOIs")`) sind zunächst erwartungsgemäß spitzer als normal und rechtsschief. Am ersten Tag sind die Verteilungen der gegen Ende des Durchgangs gespielten Tonleitern (hellgrüne bis gelbe Linien) flacher und breiter als früher gespielte Tonleitern (blaue bis dunkelgrüne Linien). Das ist ein Hinweis auf eine Korrelation von laufender Tonleiterzahl und Streubreite der IOIs und könnte von zunehmender Ermüdung der PatientInnen herrühren. Ein Blick auf `r kfigr::figr("OverviewTabIOIs", type = TABREF)` zeigt aber, dass in dieser ersten Bedingung besonders viele Tonleitern gespielt wurden, weshalb in den anderen Bedingungen (abgesehen von d43, follow-up) keine gelben Kurven sichtbar sind. In der Follow-up-Bedingung ist diese Verbreiterung der Verteilungskurven bei späteren Tonleitern allerdings nicht sichtbar.
...
if (interactive()) {
  SDs <- dplyr::summarize(dplyr::group_by(mdata,
                                          ID, Day.0, Time, Ascend, Scale),
                          SD = sd(IOI),
                          .groups = "drop_last")
  SDs <- dplyr::mutate(dplyr::group_by(SDs,
                                       ID, Day.0, Time, Ascend),
...
ggplot2::ggplot(dplyr::group_by(n_scales,
  ...
  ggplot2::facet_grid(cols = dplyr::vars(Day.0)) +
...
In `r kfigr::figr("NumberOfScalesBoxPlot")` entspricht jeder der hellblauen Punkte einer/m PatientIn. An Tag d15 sind wegen des Datenverlustes bei P05 nur vier statt fünf Punkte gezeigt.
...
Das Modell soll einen Populationsparameter, nämlich *Tag*, schätzen.
...
(form_Day__1_ID <- brms::bf(SD ~ 0 + Intercept + Day.0 +
                              (1 | ID), 
                       family = brms::skew_normal()))
...
Da nun eine Steigung für die Variable *Tag* geschätzt werden soll, muss dieser eine Prior erhalten.
...
Es gibt anscheinend in der Vorher-Messung bei der mittleren Abwärtstonleiter einen schwachen Tageseffekt.
...
(form_Day_Time__1_ID <- brms::bf(SD ~ 0 + Intercept + Day.0 + Time +
                              (1 | ID), 
                       family = brms::skew_normal()))
...
(form_Day_Time_Ascend__1_ID <- brms::bf(SD ~ 0 + Intercept + Day.0 + Time + Ascend +
                              (1 | ID), 
                       family = brms::skew_normal()))
...
(form_Day_Time_Ascend_Scale__1_ID <- brms::bf(SD ~ 0 + Intercept + Day.0 + Time + Ascend + Scale.c +
                                                (1 | ID), 
                                              family = brms::skew_normal()))
...
(form_Day_x_Time_Ascend_Scale__1_ID <- brms::bf(SD ~ 0 + Intercept + Day.0 * Time + Ascend + Scale.c +
                                                (1 | ID), 
                                              family = brms::skew_normal()))
...
(form_Day_x_Time_x_Ascend_Scale__1_ID <- brms::bf(SD ~ 0 + Intercept + Day.0 * Time * Ascend + Scale.c +
                                                (1 | ID), 
                                              family = brms::skew_normal()))
...
(form_Day_x_Time_x_Ascend_Scale__1_ID__1_Day <- brms::bf(SD ~ 0 + Intercept + Day.0 * Time * Ascend + Scale.c +
                                                    (1 | ID) +
                                                    (1 | Day.0), 
                                                  family = brms::skew_normal()))
...
(form_Day_x_Time_x_Ascend_Scale__1_ID__1_Day__1_Time <- brms::bf(SD ~ 0 + Intercept + Day.0 * Time * Ascend + Scale.c +
                                                                   (1 | ID) +
                                                                   (1 | Day.0) +
                                                                   (1 | Time), 
                                                                 family = brms::skew_normal()))
...
(form_Day_x_Time_x_Ascend_Scale__1_ID__1_Day__1_Time__1_Ascend <- brms::bf(SD ~ 0 + Intercept + Day.0 * Time * Ascend + Scale.c +
                                                                   (1 | ID) +
                                                                   (1 | Day.0) +
                                                                   (1 | Time) +
                                                                   (1 | Ascend), 
...
`r kfigr::figr("Day_x_Time_x_Ascend_Scale__1_ID__1_Day__1_Time__1_Ascend_CondPlot")`F legt nahe, dass die in @Jabusch2004 erwähnten Unterschiede in der Spielregelmäßigkeit zwischen Aufwärts- und Abwärtstonleitern auch im vorliegenden Datensatz existieren.

```{r}
# brms::hypothesis(m_Day_Time_Ascend_Scale__1_ID,
#                  # "Ascend1 = Intercept + Day.L + Day.Q + Timepost + Scale.c")
#                  "Intercept = 0")
```
...
Die winzige Stichprobengröße wäre eine mögliche Erklärung, warum das theoretische passendere Modell (*Tag x Zeit x Aufwärts + Skala + (1|ID) + (1|Tag) + (1|Zeit)*) die Daten nicht am besten erklärt, da kleine Stichproben häufiger eine sehr große Varianz aufweisen und deshalb kein statistisch relevanter Unterschied zwischen Experimentalbedingungen feststellbar ist. Eine weitere Erklärung ist, dass zwischen den den Populationseffekten wirklich keine Interaktion existiert. Es ist jedoch nicht möglich festzustellen, welche der beiden Möglichkeiten die richtige Erklärung ist.

Die Modellierungsergebnisse im Abschnitt "[Tag x Zeitpunkt x Aufwärts + Skala + (1 | ID) + (1 | Tag) + (1 | Zeitpunkt) + (1 | Aufwärts)]" zeigen, dass es in den vorliegenden Daten keinen Hinweis auf eine unmittelbare Wirkung der tDC-Stimulation gibt, da es an keinem der drei Stimulationstage Vorher-Nachher-Effekte zu geben scheint. Es gibt aber eine leichte Verbesserung der Spielunregelmäßigkeit zwischen dem ersten und dem zweiten Tag (`r kfigr::figr("Day_x_Time_x_Ascend_Scale__1_ID__1_Day__1_Time__1_Ascend_CondPlot")`E). 
...
Die Aufschlüsselung nach *Zeitpunkt* und *Aufwärts* zeigt schließlich (`r kfigr::figr("Day_x_Time_x_Ascend_Scale__1_ID__1_Day__1_Time__1_Ascend_CondPlot")`G), dass es eine divergente Entwicklung bei der Spielverbesserung gab, insofern als .
