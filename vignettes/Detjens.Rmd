...
if (interactive()) {
  ...
  mdata <- dplyr::mutate(mdata,
                         ...
                         Time = factor(Time, # Time deliberately not ordered, as for modelling 'foolow-up' needs
...
Die Verteilungen der IOIs in den sieben Bedingungen (`r kfigr::figr("OverviewPlotIOIs")`) sind zunächst erwartungsgemäß spitzer als normal und rechtsschief. Am ersten Tag sind die Verteilungen der gegen Ende des Durchgangs gespielten Tonleitern (hellgrüne bis gelbe Linien) flacher und breiter als früher gespilte Tonleitern (blaue bis dunkelgrüne Linien). Das ist ein Hinweis auf eine Korrelation von laufender Tonleiterzahl und Streubreite der IOIs und könnte von zunehmender Ermüdung der PatientInnen herrühren. Ein Blick auf `r kfigr::figr("OverviewTabIOIs", type = TABREF)` zeigt aber, dass in dieser ersten Bedingung besonders viele Tonleitern gespielt wurden, weshalb in den anderen Bedingungen (abgesehen von d43, follow-up) keine gelben Kurven sichtbar sind. In der Follow-up-Bedingung ist diese Verbreiterung der Verteilungskurven bei späteren Tonleitern allerdings nicht sichtbar.

***

`r kfigr::figr("OverviewTabIOIs")`. *Lage- und Streuparameter der Inter-onset Intervalle von P01, P02, P03 und P05 nach Tag und Messzeitpunkt (in ms). Die Spalte* N *listet für jede Bedingung die Anzahl der in die Ermittlung der Statistiken eingeflossenen IOIs.*
...
`r kfigr::figr("OverviewTabIOIs")` zeigt einige Interessante Eigenschaften des Datensatzes: 1. sind in der Spalte *Min* die Hälfte der Einträge einstellig, der Rest befindet sich im niedrigen zweistelligen Bereich, d.h. dass es extrem kurze Dauern zwischen aufeinanderfolgenden Tönen gab. Und zwar so kurz, dass einige dieser Töne psychophysisch eigentlich zeitlich gar nicht mehr zu trennen sein können. 2. zeigt die Spalte *Max*, dass es auch IOIs gab, die zeitlich mehr als das doppelte des nominellen IOI von der vorhergehenden Note entfernt waren, also eigentlich hätte zum Zeitpunkt ihres Anschlags schon die nächste Taste angeschlagen gewesen sein müssen. 3. ist in den Spalten *Mean* und *Median* zu sehen, dass die (rohen) Lageparameter alle deutlich und systematisch unterhalb des zu erwarteten IOIs von 125 ms liegen, die PatientInnen also alle zu schnell spielten. Am ehesten zu erklären wäre diese Diskrepanz durch eine fehlerhafte Metronomspur im MIDI-Sequencer. Die unterschiedlichen Anzahlen ausgewerteter Tonleitern in der Spalte *N* ist erklärbar durch den Datenverlust bei PatientIn P05, nur am ersten Tag vor der Stimulation sind ca. 100 Tonleitern mehr gespielt worden als in den folgenden Bedingungen ohne Datenverluste.
...
Auch bei P04 gab es das Phänomen, dass später gespielte Tonleitern flachere Verteilungen aufweisen. Allerdings wird auch deutlich, dass PatientInnen nach unterschiedlich vielen Tonleitern dieses Phänomen zeigen, und man deshalb zur genaueren Untersuchung die keweils letzten zehn oder 15 Tonleitern aller PatientInnen miteinander vergleichen müsste.
...
In `r kfigr::figr("NumberOfScales")` entspricht jeder der hellblauen Punkte einer/m PatientIn. An den Tagen d15 und d43 sind wegen des Datenverlustes bei P05 nur vier statt fünf Punkte gezeigt.
...
Der Shapiro-Wilks-Test [`r kfigr::figr("ShapiroTab", type = TABREF)`, @Razali2011] testet die Nullhypothese, dass eine Variable normalverteilt ist. Ist der resultierende *p*-Wert größer als $\alpha$ (hier: 0,05), kann die $H_0$ nicht zurückgewiesen werden. Ist der *p*-Wert aber kleiner, muss man mit der gewählten Irrtumswahrscheinlichkeit davon ausgehen, dass die Variable nicht normalverteilt ist. 

***

`r kfigr::figr("ShapiroTab")`. *Ergebnisse des Shapiro-Wilks-Tests für die Verteilung der SD-Werte jedes/r PatientIn in allen Bedingungen. Die Spalte* N *enthält die Anzahl der fehlerfreien Tonleitern jedes/r TeilnehmerIn in der jeweiligen Bedingung.*
...
Im nächsten Schritt wird häufig ein Populationseffekt (oft auch Fester Effekt genannt) hinzungefügt, wie z.B. ein Gruppierungsfaktor. In einem anschließenden Modellierungsschritt fügt man eventuelle Interaktionen zwischen Haupteffekten hinzu, sodann können noch eine weitere Kovariate zum Modell hinzugefügt werden. Schließlich folgen eventuelle Gruppen-spezifische Effekte, sogenannte variierende Interzepte und Steigungen, bis das Modell schließlich "komplett" ist.
...
Am Ende der Modellierung steht immer der Vergleich der verschiedenen Modelle, um entscheiden zu können, welches dasjenige ist, das die Daten am besten erklärt. Da komplexere Modelle die Daten immer besser erklären als einfachere Modelle, wird beim Vergleichen eine Strafe erteilt, die umso größer ist, je komplexer ein Modell ist. Das hat zur Folge, dass es einen Kompromiss gibt aus Erklärungsgüte und Modellkomplexität. Würde man diesen Kompromiss nicht eingehen, würde immer das komplexeste Modell "gewinnen". Da aber immer noch komplexere Modelle denkbar sind, würde man unglaublich komplexe Modelle bauen, die aber den Gedanken des Modells ad absurdum führen, nämlich dass ein Modell eine handhabbare, vereinfachende Erklärung eines komplexen Zusammenhangs darstellt. Oder man würde eine willkürliche Stoppregel einführen ("kein Modell darf mehr als fünf erklärende Variablen haben"). Beide Vorgehensweisen würden aber beim Erkenntnisgewinn nicht helfen, da ersteres nicht beim Erklären und Verstehen hilft und letzteres möglicherweise immer noch zu komplex ist, oder eben nicht komplex genug, um die Daten hinreichend zu erklären.
...
Populationseffekte        Interzept                 Schätzt mittlere *SD* am        Dies' ist die Referenz-
                                                    ...
                                                    in der <u>ersten</u> Tonleiter  dieser Referenz dar

                          Tag                       Schätzt Unterschied zu          Wenn es einen tDCS-
                                                    'Interzept' am <u>zweiten</u>,  Effekt gibt, der eine
                                                    <u>dritten</u> und              Woche überdauert, dann 
                                                    <u>vierten</u> Tag              sollte der Schätzer $\neq$
                                                                                    0 sein
                          ...
                          Spielrichtung             Schätzt *SD*-Unterschied zu     etc. pp.
                                                    'Interzept' bei 
...
Gruppeneffekte            Individuum                Je ein Interzept für jedes 
                          ...
                          Spielrichtung             Jedes Individuum erhält ein 
                                                    variierendes Interzept für 
                                                    'Spielrichtung'
...
Die Populationseffekte sind hier nicht nur einfache Eingabevariablen, sondern auch als Interaktionen im kompletten Modell enthalten, so dass (theoretisch) an verschiedenen Messtagen unterschiedliche Steigungen z.B. für vor und nach der Intervention möglich sind. Damit Interaktionen stabil geschätzt werden können, sind sehr viel größere Stichproben erforderlich, als für einfache Effektschätzer. Die vorliegenden Daten von fünf PatientInnen werden deshalb keine sinnvollen Interaktionseffekte schätzbar machen. Umso wichtiger sind deshalb die Gruppen-Interzepte für *Individuum*, *Messtag*, *Messzeitpunkt*, *Spielrichtung* und *Skalennummer*.

Das komplette Modell soll mit den Variablen *Messtag*, *Messzeitpunkt*, *Spielrichtung* und fortlaufender *Tonleiternummer* die Varianz in den Standardabweichungen der Inter-onset Intervalle erklären. An den Tagen d1 bis d15 wurde jeweils stimuliert. Wenn die Stimulation eine Verringerung der Standardabweichung der *IOI*s zur Folge hat, die mindestens eine Woche anhält, ist ein Effekt für *Tag* kleiner 1 zu erwarten. Das gleiche auf einer kürzeren Zeitachse für *Messzeitpunkt* (prä|post). Es gibt anekdotische Hinweise auf individuell unterschiedlich betroffene Spielleistung des Tonleiter-auf- und -abwärtsspiels (Tab. 1 in @Jabusch2004)); sollten sich diese Unterschiede in den Daten bestätigen, würde für den Schätzer *Spielrichtung* ebenfalls ein Wert $\neq$ 0 erwartet. Auch auf bei Dystonikern verstärkte Ermüdungserscheinungen gibt es Hinweise, denen durch die Modellierung des Parameters Tonleiternummer Rechnung getragen wird.

Der Daten der Follow-up-Messung müssen aus der Analyse ausgeschlossen werden, da es sonst zur Konfundierung von Messtag und Messzeitpunkt kommt. Die Ergebnisse der modellierten Ergebnisse kann hinterher aber mit den Rohwerten der mittleren SDs im Follow-up vergleichen werden.

```{r, message=FALSE}
SDs_reduced <- dplyr::filter(SDs,
                             Day != "d43")
SDs_reduced <- dplyr::mutate(SDs_reduced,
                             Scale = as.numeric(levels(Scale))[Scale])
...
Das Null-Modell schätzt den Mittelwert der Standardabweichungen über alle PatientInnen, Tage, Messzeitpunkte, Spielrichtungen und Skalen und nimmt als Datenstruktur nur an, dass es Wiederholungsmessungen gibt, jedes Individuum also mehr als einmal gemssen wurde. Links der Tilde (`r kfigr::figr("form_null", type = EQREF)`) steht die zu erklärende Variable, hier die individuelle Variation *SD* um den nominellen *IOI*. Die Tilde wird gelesen als "wird erklärt durch". Dann folgt die rechte Seite der Gleichung: Sie besteht in diesem Modell nur aus zwei Termen, nämlich einer `1` und `(1 | ID)`. Die `1` bedeutet, dass das Modell ein Interzept schätzen soll, also den Punkt, wo die Regressionsgerade die Y-Achse schneidet, wo also gilt: $x = 0$. Wenn die Regressionsgleichung keine weiteren Populationparameter enthält, wird keine Steigung geschätzt; diese wird also als Null angenommen und verläuft waagerecht. Damit entspricht bei diesem Modell das Interzept dem Mittelwert über alle Standardabweichungen. Der geklammerte Term ist die Anweisung, für jedes Individuum (`ID`) ein Interzept zu schätzen und kennzeichnet gleichzeitig alle Werte von *SD*, die von ein und demselben Individuum *ID* stammen, als statistisch voneinander abhängig. Bei geklammerten Termen handelt es sich immer um gruppenspezifische Parameter (manchmal zufällige Effekte genannt). Bei der Schätzung von Populationseffekten in einem Modell ohne Gruppenparameter tragen alle Individuen gleichviel zur Schätzung der Parameter bei. Ein gemischtes Modell, wie hier dem Null-Modell, bei dem sowohl Populationsparameter (hier das Interzept) als auch Gruppenparameter (hier die individuellen Interzepte) geschätzt werden, gewichtet weit vom Populationsparameter entfernte Gruppenparameter weniger stark als solche, die näher am Populationsparameter gelegen sind. 
...
Die eigentlichen Ergebnisse sind in den Abschnitten *Group-Level Effects* und *Population-Level Effects* aufgeführt. Gruppenspezifische Effekte wurden nur für die Gruppe `ID` geschätzt, die mittlere Standarabweichung der Interzepte betrug 2.18 ($\pm$ 0.45) ms. Das Populationsinterzept wurde auf 19.04 ($\pm$ 1.01) ms geschätzt, also deutlich über der von @Jabusch2004 ermittelten Schwelle zu dystonem Spiel.
...
Die posterioren Verteilungen der Parameter-Schätzer in der linken Spalte von `r kfigr::figr("null_DiagnPlot")` verdeutlichen, dass Bayes'sche Modelle keine Punktschätzer liefern, wie in der traditionellen Frequentisten-Statistik ("Die SD der PatientInnen betrug im Mittel 19.04 ms $\pm$ 1.01"), sondern Wahrscheinlichkeitsverteilungen, die ihren Mittelwert bei z.B. 19.04 ms hatte; die Streubreite dieser Wahrscheinlichkeitsverteilung betrug 1.01 ms. Das bedeutet, dass nicht behauptet wird, dass ein Mittelwert (19.04 ms) mit einer gewissen (Un-)Sicherheit (1.01 ms) bestimmt worden ist, sondern dass der wahre Wert mit größerer Wahrscheinlichkeit im zentralen als im peripheren Bereich einer $\cal{N}(19.04, 1.01)$-Verteilung liegt.
...
(form_Day__1_ID <- brms::bf(SD ~ 1 + Day +
                              (1 | ID), 
                       family = brms::skew_normal()))
...
(form_Day_Time__1_ID <- brms::bf(SD ~ 1 + Day + Time +
                              (1 | ID), 
                       family = brms::skew_normal()))
...
brms::conditional_effects(m_Day_Time__1_ID)
```

### Tag + Time + Ascend + (1 | ID)
...
(form_Day_Time_Ascend__1_ID <- brms::bf(SD ~ 1 + Day + Time + Ascend +
                              (1 | ID), 
                       family = brms::skew_normal()))
...
brms::conditional_effects(m_Day_Time_Ascend__1_ID)
```


### Tag + Time + Ascend + Scale + (1 | ID)
...
(form_Day_Time_Ascend_Scale__1_ID <- brms::bf(SD ~ 1 + Day + Time + Ascend + Scale +
                                                (1 | ID), 
                                              family = brms::skew_normal()))
...
brms::conditional_effects(m_Day_Time_Ascend_Scale__1_ID)
```

## Modell-Vergleich

```{r}
brms::loo_compare(m_null,
                  m_Day__1_ID,
                  m_Day_Time__1_ID,
