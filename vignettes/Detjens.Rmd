...
`r kfigr::figr("OverviewPlotIOIsP04", type = FIGREF)`. *Dichtefunktionsschätzung der Inter-onset Intervalle aller von P04 gespielten Tonleitern, getrennt nach* Zeit *(pre, post, follow-up) und* Tag *(1, 8, 15 und 43).*
...
Auch bei PatientIn P04 liegen Mittelwert und Median der IOIs sehr nahe bei 120 ms, was noch weniger zu ihrem/seinem nominellen IOI von 150 ms passt.
...
`r kfigr::figr("OverviewPlotSDs", type = FIGREF)`. *Dichtefunktionsschätzung der Standardabweichungen der Inter-onset Intervalle aller gespielten Tonleitern, getrennt nach* Zeit *(pre, post, follow-up) und* Tag *(1, 8, 15 und 43).*
...
`r kfigr::figr("CompleteModelTab")`. *Die bekannten Varianzquellen im vorliegenden Versuchsdesign. Die Spalte* Bedeutung *führt die Bedeutung eines* Effektes *im kompletten Model auf; in Teilmodellen, wie sie im Text besprochen werden, ändert sich die Bedeutung teilweise.*
...
Populationseffekte        Interzept                 Schätzt mittlere *SD* am        Dies ist die Referenz-
                          ...
                          Tag                       Schätzt Unterschied zu          Wenn es einen tDCS-
                                                    'Interzept' am <u>achten</u>    Effekt gibt, der eine
                                                    und <u>15.</u> Tag              Woche überdauert, dann 
                                                                                    sollte der Schätzer $\neq$
                                                                                    0 sein
...
Die Zusammenfassung der festen Effekte aus `r kfigr::figr("Day_x_Time_x_Ascend_Scale__1_ID__1_Day_summary")` werden im Auszug erneut in `r kfigr::figr("FixedEffectsWinner", type = TABREF)` gezeigt. 

***

`r kfigr::figr("FixedEffectsWinner", type = TABREF)`. *Die festen Effekte des Gewinner-Modells.*

```{r FixedEffectsWinner, anchor=TABREF}
knitr::kable(brms::fixef(m_Day_x_Time_x_Ascend_Scale__1_ID__1_Day))
```

Die Schätzer der festen Effekte deuten außer bei der Spielrichtung nicht auf bedeutsame Einflüsse der experimentellen Faktoren auf die Gleichmäßigkeit des Tonleiterspiels hin; ein Interaktionsplot verdeutlicht das (`r kfigr::figr("FixedEffectsWinner", type = FIGREF)`). Dort zeigt sich deutlich, dass die posterioren Wahrscheinlichkeiten der Schätzer sehr stark überlappen, nur die Spielrichtung könnte leichte Effekte aufweisen.

```{r FixedEffectsWinnerPlot, anchor=FIGREF}
emmeans::emmip(m_Day_x_Time_x_Ascend_Scale__1_ID__1_Day, 
      Ascend ~ Day | Time, 
      ylab = "Standard deviation of IOIs (in ms)",
      facetlab = "label_both") + 
  ggthemes::scale_color_colorblind()
```

`r kfigr::figr("FixedEffectsWinnerPlot")`. *Der Interaktionsplot zeigt für jede Bedingung den Median der zugehörigen posterioren Wahrscheinlichkeitsverteilung mit dem 95%-HPD-Intervall.*

***

Die geschätzten Mediane der posterioren Wahrscheinlichkeiten aller Bedingungskombinationen des Gewinner-Modells sind in `r kfigr::figr("EstMeansWinner", type = TABREF)` gezeigt.

***

`r kfigr::figr("EstMeansWinner")`. *Geschätzte Mediane aller Bedingungskombinationen des besten Modells. Die Spalten* Ascend, Day *und* Time *listen die jeweiligen Kombinationen der experimentellen Faktoren Aufsteigend, Tag und Zeitpunkt während der mittleren Tonleiter. Die Spalte* emmean *enthält die geschätzten Mediane, die Spalten* lower.HPD *und* upper.HPD *stellen die unteren und oberen Werte der höchsten posterioren Verteilungsdichte (Highest Posterior Density) dar.*

```{r EstMeansWinner, anchor=TABREF}
emm <- emmeans::emmeans(m_Day_x_Time_x_Ascend_Scale__1_ID__1_Day, 
                        specs = list(pairwise ~ Ascend | Day + Time))
knitr::kable(emm$`emmeans of Ascend | Day, Time`)
```

`r kfigr::figr("PairwiseDiffs", type = TABREF)` stellt die Differenzen der Bedingungsmediane samt zugehöriger unteren und oberen HPD-Intervallen dar.

***

`r kfigr::figr("PairwiseDiffs")`. *Post-hoc-Differenzen der Bedingungen. Wenn ein HPD-Intervall den Wert 0 nicht überstreicht, kann davon ausgegangen werden, dass die Intervention einen Effekt zeitigte.*

```{r PairwiseDiffs, anchor=TABREF}
knitr::kable(emm$`pairwise differences of Ascend | Day, Time`)
```

Tatsächlich zeigen die paarweisen post-hoc-Tests in `r kfigr::figr("PairwiseDiffs")` Unterschiede zwischen Aufwärts- und Abwärtsspielrichtung am ersten und 15. Tag vor der Stimulation, sowie am ersten und 15. Tag nach der Stimulation. Die Spalte *estimate* zeigt aber auch, dass sich die Mediane der Spielrichtungen um bescheidene 2 bis 4 ms unterschieden. 

### Langzeitwirkung

Nachdem in den Daten weder ein Vorher-Nachher-Effekt von tDCS auf die Regelmäßigkeit des Tonleiterspiels gefunden wurde, noch einer zwischen den einzelnen Stimulationstagen, ist es müßig, einen Langzeiteffekt zu untersuchen.
...
Die Standardabweichungen der Aufwärts- und Abwärtstonleitern entwickeln sich jedoch im Laufe der dreiwöchigen Stimulation deutlich auseinander, was sich in (`r kfigr::figr("Day_x_Time_x_Ascend_Scale__1_ID__1_Day_CondPlot")`G) so darstellt, als ob es bei den Abwärtstonleitern kaum Veränderungen gibt, sich die Regelmäßigkeit der Aufwärtstonleitern aber deutlich verbessert. `r kfigr::figr("FixedEffectsWinnerPlot")` zeigt aber deutlich, dass auch dieser Effekt recht schwach ist.
